...
function updateList(bookmarks, prev, moved=false) {
    if (!moved) {
        ...
        if (bookmarks[0]) {parentFolderID = bookmarks[0].parentId}
    }
    ...
    else if (container.firstElementChild) { // folder might be empty
...
function open(e, highlightedElem) {
    if (highlightedElem.getAttribute("type") === "bookmark") {
        ...
        browser.bookmarks.getChildren(parentFolderID)
        .then(bookmarks => updateList(bookmarks, false));
    }
}

function backward(highlightedElem) {
    browser.bookmarks.get(parentFolderID)
    .then((bookmarks) => {
        // we don't want to do anything if we are inside root
        // root has no parent so we check if our current folder has a parentId
        
        const grandparentID = bookmarks[0].parentId;
        // if not root
        if (grandparentID) {
            browser.bookmarks.getChildren(grandparentID)
...
function deleteBookmark(highlightedElem) {
    ...
    surelabel.textContent = "Are you sure you want to delete these bookmarks/folders? y/n";
...
function newFolder(e, highlightedElem) {
    ...
    inputContainer.addEventListener("keydown", e => {
        if (e.key === "Enter") {
            ...
            browser.bookmarks.create({title:`${input.value}`, parentId:`${parentFolderID}`})
            .then(bk => {
...
function search(e) {
    ...
    searchBox.addEventListener("keydown", e => {
        if (e.key === "Enter") {
            browser.bookmarks.search(searchBox.value)
            .then(bookmarks => {
...
function select(bkID) {
    ...
    if (selectionList.includes(bkID, 0)) {selectionList.splice(index, 1)}
    else {selectionList.push(bkID)}
}
...
function moveTo() {
    ...
    browser.bookmarks.get(selectionList)
...
function edit(e, highlightedElem) {
    ...
    cont.addEventListener("keydown", e => {
        if (e.key === "Enter") {
            ...
            browser.bookmarks.update(highlightedElem.getAttribute("bkID"), {title: title.value})
            if (highlightedElem.getAttribute("bookmark")) {
                highlightedElem.setAttribute("url", url.value);
                browser.bookmarks.update(highlightedElem.getAttribute("bkID"), {url: url.value})
            }
            cont.remove();
            document.addEventListener("keydown", handleKeyDown);
        }
...
.then(bookmarks => {
    ...
    parentFolderID = rootFolder.id; // in case toolbar folder is empty
